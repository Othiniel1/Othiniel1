              ...
              {mesProduits.map((prod) => (
                <div key={prod.id} className="bg-slate-50 p-3.5 rounded-xl border border-slate-200 space-y-2">
                  <div className="flex justify-between items-start">
                    ...
                    <span className={`px-2 py-0.5 rounded font-extrabold text-[10px] flex items-center gap-1 ${
                      prod.stock < 3 ? 'bg-rose-100 text-rose-800 border border-rose-300' : prod.stock < 10 ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800'
                    }`}>
                      {prod.stock < 3 && <AlertCircle className="w-3 h-3 text-rose-600" />}
                      Stock: {prod.stock} {prod.stock < 3 && '(Critique)'}
                    </span>
                  </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-sm w-full p-5 space-y-3 shadow-xl border border-slate-200 text-xs">
            ...
            <p className="text-slate-600">
              Voulez-vous réinitialiser l&apos;état du prototype SOKU ? Toutes les modifications seront restaurées.
            </p>
            <div className="flex gap-2 justify-end pt-2">
              ...
              <button
                onClick={() => {
                  ...
                  setNotification('État du prototype réinitialisé.');
                }}
